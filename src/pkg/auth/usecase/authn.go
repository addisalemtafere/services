...
func (uc Usecase) InitPhoneAuth(token, prefix, number string) (*entity.PhoneAuth, error) {
	...
	// Send code
	go func() {
		uc.sms.SendSMS(phone.String(), fmt.Sprintf("Your KENO verification code is: %s. Do not share this code with anyone. Our employees will never ask for the code.", strconv.Itoa(otp)))
	}()
